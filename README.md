...
下一轮已确认仅做文字页面的长文分段、逐段显示、整体进度和完整双语导出；保存恢复、历史任务、单段重试及语音漏词修复暂缓。功能尚未实施，交接见 [下一阶段任务书](docs/下一阶段-纯文本长文翻译任务书.md) 与 [新对话启动提示词](docs/下一阶段-开发对话启动提示词.md)。
...
当前应用为 **0.2.2 / build 6**，位于 [本地翻译器.app](dist/M0-20260923-214836-release/本地翻译器.app)，压缩包位于 [本地翻译器-M0.zip](dist/M0-20260923-214836-release/本地翻译器-M0.zip)。在 Finder 中双击应用即可。它面向当前 Apple Silicon Mac，要求 macOS 26 或更新，采用本机临时签名，未做公开分发或 Developer ID 公证。原始 60 分钟长测对应 build 5；build 6 的纯标点修复已针对性复测，未重跑一小时推理。
