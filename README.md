...
当前交付为 **0.2.1 原生 Mac 流式字幕测试应用**和命令行验证程序。应用提供文字翻译、文档/图片提取及选段翻译、麦克风/文件连续字幕、任务保存与补译、录后重新校对、本机资源设置。完整首版仍需阅读、文档、实时质量及长课堂恢复等全部通过验收。
...
当前应用位于 [本地翻译器.app](dist/M0-20260917-115734-release/本地翻译器.app)，压缩包位于 [本地翻译器-M0.zip](dist/M0-20260917-115734-release/本地翻译器-M0.zip)。在 Finder 中双击应用即可。它面向当前 Apple Silicon Mac，要求 macOS 26 或更新，采用本机临时签名，未做公开分发或 Developer ID 公证。
...
新的录音任务保存在 `~/Library/Application Support/LocalTranslator/Recordings`；翻译失败或关闭后可打开任务补译。识别中断仍需重新处理原音频，或使用已保存音频生成校对版，不能无缝续录。真人麦克风、设备中断及 60–90 分钟课堂尚未验收。文字/文档和“原生引擎对照”仍使用原有临时目录。
