...
当前交付为 **0.2.3 原生 Mac 纯文本长文翻译测试应用（build 7）**和命令行验证程序。应用提供长文自动分段、逐段对照、完整进度与双语 TXT 导出、文档/图片提取及选段翻译、麦克风/文件连续字幕、任务保存与补译、录后重新校对、本机资源设置。完整首版仍需阅读、文档、实时质量及长课堂恢复等全部通过验收。

2026-09-22 新增 VAD 前置音频、独立录音落盘、暂停/继续、检查点恢复和字幕分页。09-23 已完成真实模型恢复与 60 分钟重复 TED 原速回放：音频完整、841 次翻译全部收尾，但仍有漏词、重复识别和时间戳异常。修复纯标点字幕后交付 build 6，45 项 Python 回归及完整事件重放通过；本轮稳定性结论、质量限制及证据见 [0.2.2 开发记录](docs/0.2.2录音与恢复开发记录.md)。
...
2026-09-24 已完成文字页面的长文分段、逐段显示、整体进度和完整双语导出。实机使用本地 HY-MT2 1.8B 翻译 14,130 字符英文文章，9 段全部完成；停止后的导出仍保留全部原文，中译英短文也已通过流程验证。Swift 22 项回归通过。保存恢复、历史任务、单段重试及语音漏词修复继续暂缓，详见 [0.2.3 开发记录](docs/0.2.3纯文本长文开发记录.md) 与 [本轮任务书](docs/下一阶段-纯文本长文翻译任务书.md)。
...
当前应用为 **0.2.3 / build 7**，位于 [本地翻译器.app](dist/M0-20260924-131335-release/本地翻译器.app)，压缩包位于 [本地翻译器-M0.zip](dist/M0-20260924-131335-release/本地翻译器-M0.zip)。在 Finder 中双击应用即可。它面向当前 Apple Silicon Mac，要求 macOS 26 或更新，采用本机临时签名，未做公开分发或 Developer ID 公证。0.2.2 的语音长测仍为历史证据，本轮未重跑长时间语音推理。

| 页面 | 已接入的交互 |
| --- | --- |
| 文字翻译 | 粘贴整篇文章、英中/中英与模型选择、自动分段、逐段对照、完整进度、停止、复制已有译文及完整双语 TXT 导出 |
...
新的录音任务保存在 `~/Library/Application Support/LocalTranslator/Recordings`；翻译失败或关闭后可打开任务补译。识别中断可继续处理已保存音频，或生成独立校对版。实体麦克风已完成环境音采集及暂停/继续检查；设备拔插、真实休眠、自然课堂质量与完整长时验收的状态见开发记录。文字任务仅保留于当前窗口，退出后不恢复；请主动复制或导出。文档和“原生引擎对照”仍使用原有临时目录。
...
脚本执行 Release 构建，将界面和工作进程、图标及依赖声明打包并验证签名，每次写入新的 `dist/M0-时间-release/` 目录。文字页面通过异步本机请求串行翻译，分段和导出拼接在后台执行；文档与原生语音仍通过独立工作进程调用 `translator-m0`。原有命令行入口和测试仍然保留。
...
在应用「文字翻译」页粘贴文章，选择方向及已有模型，点击「开始翻译」。整篇输入不再受 8,000 字符限制；程序依次按段落、句子、词边界和 Unicode 字符边界分段，按顺序显示每段原文、完成译文与核对提示。上方列出总段数、已完成、处理中、待处理、失败和中断数量。

「停止」保留完整完成的段落，取消当前请求且不再发起后续段；「重新开始」会从第一段重译，不提供续跑或单段重试。运行时禁止修改原文、方向和模型，修改这些设置会清除旧任务显示。当前任务可随时「复制已有译文」或「导出双语 TXT」：复制仅包含完成译文，TXT 包含所有原文和对应译文，失败、待处理及停止中断段分别标记。导出使用点击时快照和系统保存面板；结果不自动持久化，关闭应用前请主动保存。

命令行单次验证用法如下。先显式启动配置为仅本地模式的 Ollama，并准备选定模型；图形界面的本机资源页也可主动启动服务。程序只接受回环地址、检查已安装模型元数据、拒绝云模型标记和 HTTP 重定向，不下载模型。
...
单次模型请求统一使用保守预算：8,192 上下文、4,096 输出上限、1,024 提示预留和 512 余量；原文每个 UTF-8 字节估作一个 token，并按原文字节的两倍预留预计译文，因此每次原文最多 2,048 UTF-8 字节。这是估算，不是模型 tokenizer 的精确计数；图形界面自动分段，CLI 单次 `translate` 和文档选段超过预算时明确报错。过大的单个 Unicode 字素会明确拒绝，保留全文供导出，不强行破坏字符。

只有收到正常结束标记的响应才成为完成译文；截断、空译文、取消或中断均不能记为成功。独立响应错误可继续后续段；服务不可用或配置校验失败会停止调度，并保留待处理段。分段不会删除真实重复，但模型仍可能漏译或误译；不承诺跨段上下文、术语一致性或 Markdown 语法保护。
