...
extension ModelUtilities {
    ...
    public static func loadTokenizer(
        ...
        // LOCAL_TRANSLATOR_OFFLINE_TOKENIZER: no implicit cache search or Hub fallback.
        // Keep variant detection in WhisperKit.loadTokenizerIfNeeded(), but load resources
        // only from the application's explicitly validated tokenizer directory.
        guard let tokenizerFolder else { throw WhisperError.tokenizerUnavailable() }
        let wrapper = try await AutoTokenizerWrapper.from(modelFolder: tokenizerFolder)
        return WhisperTokenizerWrapper(tokenizer: wrapper, at: tokenizerFolder)
    }
