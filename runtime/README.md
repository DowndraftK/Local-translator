...
已验证真人 TED 文件、翻译中断恢复、采样转换、管道落盘及实体麦克风的允许权限、暂停/继续与停止保存。真实设备断开、拒绝/撤回权限、整机睡眠和自然课堂质量尚待实际验收。最新恢复及长时验证状态见 [0.2.2 开发记录](../docs/0.2.2录音与恢复开发记录.md)。没有系统声音采集、说话人识别或自动保存所有译文历史。应用仍会出现英文漏词、中文条件误译；“本次处理完成”只表示流程结束。
